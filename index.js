...
const apivhtear = 'BELI SENDIRI YA BEB'
const ZeksApi = 'apivinz'
const BarBarKey = 'BELI SENDIRI AJG'
...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511946462131:+5511946462131\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
vr = '*TIAGO*'
/******** OWNER NUMBER**********/
const ownerNumber = ["5511946462131@s.whatsapp.net"]  //ganti menjadi nomormu
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Eae @${num.split('@')[0]}\ SEJA BEM-VINDO(A) ao GRUPO *${mdata.subject}* LEIA A DESC DO GRUPO ANTES DE FAZER MERDA`
        ...
        if (isGroup && isBadWord) {
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    ...
                    return reply( "JAMAIS DE COMIDA PARA O CLOVIS 😇")
                }
            }
        }
		...
		if (!isRegistered) return reply( ind.noregis())
			...
			case 'toxic':		//UPDATE
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply(ind.wrongf())
					teks = body.slice(5)
					buffer = await getBuffer(`https://api.vhtear.com/textxgif?text=${teks}&apikey=${apivhtear}`, {method: 'get'})
					client.sendMessage(from, buffer, sticker, {quoted: mek})
					await limitAdd(sender) 
					break
		...
		case 'dadu':
			...
			break
				...
				reply(ind.wait())
                 ...
                 client.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`GELAP NI BOSS\`\`\`'})
				await limitAdd(sender)
				break
			case 'asupan':
				client.updatePresence(from, Presence.composing) 
...
break
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 」*\n\nDo grupo : ${groupName}\nRemetente : wa.me/${(sender.split('@')[0])}\nMensagem : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 」*\n\nDo grupo : ${groupName}\nRementente: wa.me/${(sender.split('@')[0])}\nMensagem : ${body.slice(6)}`)
						}
        ...
        case 'register':
                ...
                if (namaUser.length >= 30) return reply(`*por que seu nome  to longo  um nome ou um trem*`)
                if (umurUser > 20) return reply(`*SUA IDADE  MUITO VELHA*`)
                if (umurUser < 10) return reply(`*VOC TEM IDADE MUITO JOVEM*`)
		try {
		...
		ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		    ...
		    addATM(sender)
                    ...
                    addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    await client.sendMessage(from, ppimg, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
		    reply('Obrigado por se registrar')
                    addATM(sender)
                    addLevelingId(sender)
	...
	case 'hobby':
		...
		reply(ind.wait)
                ...
                case 'ytmp4':
                    ...
                    txt += `Durao : ${get_result.duration}\n`
                    txt += `Visualizar : ${get_result.view}\n`
                    txt += `Gostar : ${get_result.like}\n`
                    txt += `No gosto : ${get_result.dislike}\n`
				...
				case 'yutubdl':
					...
					if (!isRegistered) return reply(ind.noregis())
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('O URL DELE NO  VLIDO, IRMO')				
		...
		case 'play':
			...
			break
				...
				if (args.length < 1) return client.sendMessage(from, 'Código de idioma obrigatório', text, {quoted: mek})
					...
					if (args.length < 2) return client.sendMessage(from, 'Digite algo para o comando iniciar', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Texto muito grande rsrsrsrs😤')
				...
				case 'clearall':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Pedido recebido, bloquear ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
			...
			if (!isGroup) return reply(ind.groupo())
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`})
						}
						reply('*_transmissão de sucesso_* ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 𝐇𝐈𝐃𝐄𝐊𝐈 𝐁𝐎𝐓-𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 」*\n\n${body.slice(4)}`)
						}
						reply('*_transmissão de sucesso_* ')
					}
					break
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('[SUCESSO] Alterando o ícone do grupo')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('Digite o número de quem você quer colocar no grupo, burro')
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "Hideki", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Tiago (*>*)',MessageType.text, { quoted: mek} )
		const RIUGANTENG = fs.readFileSync('./sticker/RIUGANTENG');
                client.sendMessage(from, RIUGANTENG, MessageType.image, {quoted: mek, caption: '*GASOSO*'})
                 break
		break    
           	case 'setname':
                if (!isGroup) return reply(ind.groupo())
                ...
                client.sendMessage(from, 'Sucesso, descrição do grupo mudada', text, {quoted: mek})
					break
           				case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque a pessoa que você quer tirar o negro supremo')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Agora você é um negro comum :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(` @${mentioned[0].split('@')[0]} Agora é um negro comum, que peninha rs`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque a pessoa que será o novo negro supremo')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Parabens� você se tornou um negro supremo :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabens� @${mentioned[0].split('@')[0]} é o novo negro supremo`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o negro comum que você quer dar ban')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`foi de base pra largar de ser gayzinho rs, @${mentioned[0].split('@')[0]} `, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `Lista de negros supremos do grupo: *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
		...
		case 'next':
        	...
        	break
				case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Marque algum sticker para funcionar')
					reply(ind.wait())
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'QUEIJO'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
				...
				case 'nsfw':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Vaia :')
					if (Number(args[0]) === 1) {
						if (isNsfw) return reply(' *ATIVADO*')
						nsfw.push(from)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Habilitando NSFW neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Desativar NSFW neste grupo')
					} else {
						reply(ind.satukos())
					}
					break
                ...
                if (args.length < 1) return reply('Vaia :')
                if (args[0] === 'ativar') {
                    if (isLevelingOn) return reply('*O RECURSO ESTAVA ATIVO*')
                    _leveling.push(from)
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Habilite o recurso Bem Vindo / Sair neste grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('Desativar Bem Vindo / Sair neste grupo')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'clone':
					...
					if (args.length < 1) return reply(' *Marque a pessoa que você quer clonar porra* ')
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('cuzinho')
					...
					try {
						...
						mentions(`Foto do perfil atualizada com sucesso. Usando a foto do perfil do: @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply(ind.stikga())
					}
					await limitAdd(sender)
		...
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[RIUBOT]','yellow'), 'Unregistered Command from', color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
